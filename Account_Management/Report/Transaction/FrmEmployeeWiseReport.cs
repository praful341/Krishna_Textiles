...
{
    public partial class FrmEmployeeWiseReport : DevExpress.XtraEditors.XtraForm
    {
        #region "Data Member"
        ...
        private void GetData()
        {
            ...
            DTabEmployee.DefaultView.Sort = "short_name";
            DTabEmployee = DTabEmployee.DefaultView.ToTable();

            ListEmployee.InvokeEx(t =>
            {
                t.Properties.DataSource = DTabEmployee;
        ...
        private void BtnGenerateReport_Click(object sender, EventArgs e)
        {
            ...
            ReportParams_Property.employee_id = Val.Trim(ListEmployee.Properties.GetCheckedItems());
        ...
        {
            DTab = ObjReportParams.GetEmployeeWise_Report(ReportParams_Property, ObjReportDetailProperty.Procedure_Name);
        }
        ...
        private void ListEmployee_EditValueChanged(object sender, EventArgs e)
        {
            try
            {
                DataTable DeTemp = new DataTable();
                var temp = ListEmployee.Properties.GetCheckedItems().ToString().Replace(" ", "").Replace("  ", "").Trim();
                if (temp != "")
                    DeTemp = (DataTable)ListEmployee.Properties.DataSource;

                if (temp.Length > 0)
                    DeTemp = DeTemp.Select("employee_id in (" + temp + ")").CopyToDataTable();

                StrColumnName = "Employee ";
                string StrFieldName = "short_name";
                TreeView_List(DeTemp, StrColumnName, StrFieldName);
            }
            catch (Exception ex)
            {
